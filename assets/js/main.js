...
function render(meals, id){
    ...
    meals.forEach(meal => {
        ...
        card.addEventListener('click', () => {
            opencard(meal); // Mở popup
        
            // Delay 0ms để tránh bắt ngay sự kiện click đầu tiên
            setTimeout(() => {
                const handleOutsideClick = (event) => {
                    const box = document.querySelector(".pop-up__detail");
                    const element = document.getElementById("detail__food");
        
                    if (!box.contains(event.target)) {
                        element.classList.add("d-none");
                        document.removeEventListener("click", handleOutsideClick); // Gỡ bỏ sau khi đã ẩn
                    }
                };
